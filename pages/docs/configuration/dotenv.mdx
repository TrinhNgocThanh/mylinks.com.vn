...
While the default settings provide a solid foundation for a standard `docker` installation, delving into this guide will unveil the full potential of MyLinks. This guide empowers you to tailor MyLinks to your precise needs. Discover how to adjust language model availability, integrate social logins, manage the automatic moderation system, and much more. It's all about giving you the control to fine-tune MyLinks for an optimal user experience.

> **Reminder: Please restart MyLinks for the configuration changes to take effect**

Alternatively, you can create a new file named `docker-compose.override.yml` in the same directory as your main `docker-compose.yml` file for MyLinks, where you can set your .env variables as needed under `environment`, or modify the default configuration provided by the main `docker-compose.yml`, without the need to directly edit or duplicate the whole file.
...
- You can also view an example of an override file for MyLinks in your MyLinks folder and on GitHub:
    - **[docker-compose.override.example](https://github.com/TrinhNgocThanh/mylinks.com.vn/blob/main/docker-compose.override.yml.example)**
...
<OptionTable
  options={[
    ['MONGO_URI', 'string', 'Specifies the MongoDB URI.','MONGO_URI=mongodb://127.0.0.1:27017/MyLinks'],
  ]}
/>
Change this to your MongoDB URI if different. You should add `MyLinks` or your own `APP_TITLE` as the database name in the URI.
...
* `mongodb+srv://username:password@host.mongodb.net/MyLinks?retryWrites=true` (`retryWrites` is the only option you need when using the online database.)
...
To configure MyLinks for local use or custom domain deployment, set the following environment variables:
...
When deploying MyLinks to a custom domain, replace `http://localhost:3080` with your deployed URL
...
MyLinks has built-in central logging, see [Logging System](/docs/configuration/logging) for more info.

#### Log Files

* Debug logging is enabled by default and crucial for development.
* To report issues, reproduce the error and submit logs from `./api/logs/debug-%DATE%.log` at: **[MyLinks GitHub Issues](https://github.com/TrinhNgocThanh/mylinks.com.vn/issues)**
...
* `CONSOLE_JSON`: When handling console logs in cloud deployments (such as GCP or AWS), enabling this will dump the logs with a UTC timestamp and format them as JSON.
  * See: [feat: Add CONSOLE_JSON](https://github.com/TrinhNgocThanh/mylinks.com.vn/pull/2146)
...
Specify an alternative location for the MyLinks configuration file.
...
> **Note**: If you prefer MyLinks to search for the configuration file in the root directory (which is the default behavior), simply leave this option commented out.

<OptionTable
  options={[
    ['CONFIG_PATH', 'string', 'An alternative location for the MyLinks configuration file.', '# CONFIG_PATH=https://raw.githubusercontent.com/danny-avila/MyLinks/main/librechat.example.yaml'],
  ]}
/>
...
### General Config
Uncomment `ENDPOINTS` to customize the available endpoints in MyLinks.
...
<div style={{display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "column"}}>
  <div className="image-light-theme">
    <img src="https://github.com/TrinhNgocThanh/mylinks.com.vn/assets/32828263/4c51dc25-31d3-4c51-8c2a-0cdfb5a25033" style={{ width: "75%", height: "75%" }} alt="Image for Light Theme" />
  </div>

  <div className="image-dark-theme">
    <img src="https://github.com/TrinhNgocThanh/mylinks.com.vn/assets/32828263/3bc5371d-e51d-4e91-ac68-56db6e85bb2c" style={{ width: "75%", height: "75%" }} alt="Image for Dark Theme" />
  </div>
</div>
...
<OptionTable
  options={[
    ...
    ['ALLOW_SOCIAL_LOGIN', 'boolean', 'Allow users to connect to MyLinks with various social networks.','ALLOW_SOCIAL_LOGIN=false'],
...
- For more information: **[Refresh Token](https://github.com/TrinhNgocThanh/mylinks.com.vn/pull/927)**
...
> Failing to set valid values here will result in MyLinks using the unsecured password reset!
...
<OptionTable
  options={[
    ['APP_TITLE', 'string', 'App title.','APP_TITLE=MyLinks'],
...
* The birthday hat icon will show automatically on February 11th (MyLinks's birthday).
...
**Important:** If using Redis, you should flush the cache after changing any MyLinks settings.
