---
title: RAG API
description: Configure Retrieval-Augmented Generation (RAG) API for document indexing and retrieval using Langchain and FastAPI. This API integrates with MyLinks to provide context-aware responses based on user-uploaded files.
---
...
To set up the RAG API with MyLinks, follow these steps:
...
For Docker, the setup is configured for you in both the default `docker-compose.yml` and `deploy-compose.yml` files, and you will just need to make sure you are using the latest docker image and compose files. Make sure to read the [Updating MyLinks guide for Docker](/docs/local/docker#update-librechat) if you are unsure how to update your Docker instance.
...
**Important:** When using the default docker setup, the .env file, where configuration options can be set for the RAG API, is shared between MyLinks and the RAG API.
...
In a local setup, you will need to manually set the `RAG_API_URL` in your MyLinks `.env` file to where it's available from your setup.
...
When using the default docker setup, the .env file is shared between MyLinks and the RAG API. For this reason, it's important to define the needed variables shown in the [RAG API readme.md](https://github.com/danny-avila/rag_api/blob/main/README.md)
</Callout>

Here are some notable configurations:

- `RAG_OPENAI_API_KEY`: The API key for OpenAI API Embeddings (if using default settings).
    - Note: `OPENAI_API_KEY` will work but `RAG_OPENAI_API_KEY` will override it in order to not conflict with the MyLinks credential.
...
Once the RAG API is set up and running, it seamlessly integrates with MyLinks. When a user uploads files to a conversation, the RAG API indexes those files and uses them to provide context-aware responses.

**To utilize the RAG API effectively:**

1. Ensure that the necessary files are uploaded to the conversation in MyLinks. If `RAG_API_URL` is not configured, or is not reachable, the file upload will fail.
...
3. The retrieved information will be used to augment the user's prompt, enabling MyLinks to generate more accurate and contextually relevant responses.
4. Craft your prompts carefully when you attach files as the default behavior is to query the vector store upon every new message to a conversation with a file attached. 
    ...
    - ![image](https://github.com/TrinhNgocThanh/mylinks.com.vn/assets/110412045/29a2468d-85ac-40d7-90be-a945301c5729)
5. You only have to upload a file once to use it multiple times for RAG.
    ...
    - ![image](https://github.com/TrinhNgocThanh/mylinks.com.vn/assets/110412045/b40cb3d3-e6e7-46ec-bc74-65d194f55a1e)
    ...
    - ![image](https://github.com/TrinhNgocThanh/mylinks.com.vn/assets/110412045/1f27e974-4124-4ee3-8091-13514cb4cbca)
...
- Check both the MyLinks and RAG API logs for any error messages or warnings.

If the problem persists, please refer to the RAG API documentation or seek assistance from the MyLinks community on GitHub Discussions or Discord.
